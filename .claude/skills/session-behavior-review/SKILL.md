...
6. **技术债分析决策**
   - 先判断问题属于“必须立即修复的行为 bug”、 “应沉淀为技术债”、还是“无需改动，只需解释/补充运行手册”。
   - 若是技术债，必须说明：
     - 债务类型：架构边界、测试缺口、工具链可靠性、提示词/skill 约束、数据模型、运维可观测性、历史兼容。
     - 触发条件和影响范围：哪些 session/message/project 会受影响，是否会继续污染后续会话。
     - 风险等级：P0/P1/P2/P3，以及为什么不是更高或更低。
     - 决策：本轮顺手修、拆成独立任务、写入现有 debt 文档/issue、或暂不处理。
   - 不要把技术债清单当成修复本身；若决定本轮不修，必须给出可验证的后续验收口径。
   - 若仓库已有技术债文档或 issue 管理入口，优先更新既有位置；若相关 debt 已执行完，允许删除过期文档，但要避免留下重复清单。
...
8. **验证闭环**
   ...
   - 如果复盘揭示可复用的 session 行为失败模式，新增或更新评估集：
     - case fixture 放在 `tests/fixtures/session_behavior_review_cases/*.json`。
     - 测试入口放在 `tests/test_session_behavior_review_case_set.py`。
     - case 至少固定：现象、关键事实、必须识别的根因、期望修复、技术债决策、必须提到的证据、禁止误判的结论。
     - 评估集用于约束 review 质量，不替代生产代码回归测试；确定性流程错误仍要补对应代码测试。
...
技术债
- 本轮识别的技术债、风险等级、处理决策和后续验收口径。

评估集
- 新增或更新的 case 路径，以及它约束的失败模式。
