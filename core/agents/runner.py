...
2. Context：先读取 workspace/input/artifact_roots.json、project_context.json 和 project_workspace.json，
   ...
   如果问题涉及尚未加载的相关项目，调用 prepare_project_worktree 按需加载到当前 session。
   ...
   项目级 resume id 由 dispatch_to_project 按 project_workspace.agent_sessions.projects 自动管理。
...
4. Skill selection：只有遇到明确业务场景时才调用 workflow skill，例如 ONES、日志排障、
   ...
   主编排 Agent 只负责识别项目和整理上下文，必须调用 dispatch_to_project 进入项目子编排处理。
...
def _build_generic_prompt(request: AgentRequest) -> str:
    ...
    return (
        ...
        "如果问题涉及尚未加载的相关项目，调用 prepare_project_worktree，并传入 session_workspace_path。\n"
        "workspace/input/session.json 里的 agent_session_id 只属于当前主编排 Agent；"
        "派发项目 Agent 时不要把它传给 dispatch_to_project。dispatch_to_project 会按 project_name "
        "从 project_workspace.agent_sessions.projects 自动恢复项目级 session_id。\n"
        "凡是涉及已注册项目、项目代码、ONES 工单、现场日志、版本 worktree 或项目 skill 的任务，"
        "主编排只做路由与上下文整理，必须调用 dispatch_to_project，并传入 db_session_id；"
