...
2. Context：先读取 workspace/input/artifact_roots.json 和 project_context.json，必要时读取
   session_dir/session_workspace.json 了解 session 级 workspace 地图；再读取 message.json、
   session.json、history.json、media_manifest.json、skill_registry.json，只取回答当前问题所需的最小上下文。
   如果 project_context.json 包含 project_runtime，ONES/现场问题的回复必须显式包含项目名、
   版本、checkout_ref/target_branch、current_branch、execution_path/worktree、worktree 检出版本
   和检出 commit 信息。
...
class DefaultAgentPort:
    async def run(self, request: AgentRequest) -> AgentResponse:
        ...
        result = await agent_client.run(
            ...
            session_id=str(session.get("agent_session_id") or "").strip() or None,
            runtime=runtime,
...
def _build_generic_prompt(request: AgentRequest) -> str:
    ...
    return (
        ...
        "请先读取 workspace/input/artifact_roots.json 和 project_context.json；"
        ...
        "如果 project_context.json 包含 project_runtime，ONES/现场问题的最终卡片必须带上"
        "项目、版本、分支/Tag、current_branch、worktree/execution_path、worktree 检出版本和检出 commit。\n"
