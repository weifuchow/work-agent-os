...
4. Skill selection：只有遇到明确业务场景时才调用 workflow skill，例如 ONES、日志排障、
   ...
   如果消息涉及已注册项目、项目代码、ONES 工单、现场日志、版本 worktree 或项目 skill，
   先加载 main-orchestrator-project-coordination skill 作为项目协同 playbook；主编排 Agent 只负责识别项目和整理上下文，
   必须调用 dispatch_to_project 进入项目子编排处理。
   不要在主编排里直接阅读项目代码、检索项目日志或调用项目 workflow skill 完成业务分析。
   调用 dispatch_to_project 时必须显式传入 project_name、task、context；涉及 workflow 时必须显式传入 skill。
   如果消息涉及 ONES 工单、现场日志、订单/车辆执行链路、截图或附件排障，
   调用 dispatch_to_project 时必须显式传入 skill="riot-log-triage"，让项目 Agent 加载该 workflow skill。
...
def _build_generic_prompt(request: AgentRequest) -> str:
    ...
    return (
        ...
        "凡是涉及已注册项目、项目代码、ONES 工单、现场日志、版本 worktree 或项目 skill 的任务，"
        "先使用 main-orchestrator-project-coordination skill 作为项目协同 playbook；主编排只做路由与上下文整理，"
        "必须调用 dispatch_to_project，并显式传入 project_name、task、context 和 db_session_id；"
        "不要在主编排里直接检索项目文件或完成项目分析。\n"
        "如果是 ONES 工单、现场日志、订单/车辆执行链路、截图或附件排障，"
        "调用 dispatch_to_project 时必须传 skill=\"riot-log-triage\"。\n"
